    ...
    for (i = 16300; i < 16384; i++) {
        ...
        if (true) {
            const long double r2 =
                ((long double)rint((double)(rt * rt)));  // shim_rintl(rt * rt);

            if (j >= max_iter) {
                perror(
                    "nsqrt: Newton-Raphson sqrt "
...
static int stress_cpu_fibonacci(void)
{
    ...
    if (fn_res != fn) {
        perror(
            "fibonacci: fibonacci error detected, summation "
...
void rt_stats_update(rt_stats_t* stats, unsigned long runtime_us,
                     bool deadline_miss)
{
...
void rt_stats_print(const rt_stats_t* stats, const rt_task_config_t* config)
{
    ...
    printf("Deadline misses: %lu (%.2f%%)\n", stats->deadline_misses,
           (stats->iterations > 0)
               ? (100.0 * stats->deadline_misses / stats->iterations)
               : 0.0);
    printf("Period:          %lu ms\n", config->period_ms);
...
static void print_usage(const char* prog_name)
{
    ...
    printf(
        "  -d, --deadline DEADLINE Deadline in milliseconds (default: "
        "period)\n");
    printf(
        "  -r, --runtime RUNTIME   Expected runtime in microseconds (default: "
        "50000)\n");
    ...
    printf(
        "                          5: Memory - Memory-intensive random "
        "access\n");
    printf(
        "                          6: Crypto - Cryptographic hash "
        "simulation\n");
    ...
    printf(
        "  -s, --stats             Enable detailed statistics (default: "
        "enabled)\n");
    printf(
        "  -t, --timer             Use timer-based periodic execution "
        "(default: signal-based)\n");
    printf(
        "  -b, --bpf               Use BPF/TT-Sched futex-based wakeup "
        "(timpani26 mode)\n");
    ...
    printf(
        "  - Set deadline 10-20%% less than measured runtime for safety "
        "margin\n");
...
{
    /* Disable stdout buffering so logs appear immediately even if podman
     * buffers or we hang on futex */
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "p:d:r:P:a:l:stbhV", long_options,
                              NULL)) != -1) {
        switch (opt) {
            case 'p':
    ...
    if (!use_bpf) {
        if (set_realtime_priority(task_config.priority) == -1) {
            fprintf(stderr,
                    "Warning: Could not set real-time priority. Running as "
                    "normal priority.\n");
        }
    } else {
        /* Set SCHED_EXT so scx_timpani (SCX_OPS_SWITCH_PARTIAL) takes over this
         * task */
        set_sched_ext();
        printf(
            "[BPF mode] Set SCHED_EXT policy — scx_timpani will manage this "
            "task.\n");
    }
    ...
    printf("Algorithm:       %d (%s)\n", algo,
           ...
           : algo == ALGO_PRIME  ? "Prime calculation"
                                 : "Unknown");
    printf("Loop count:      %d\n", loop_cnt);
    printf("Execution mode:  %s\n", use_bpf     ? "BPF/TT-Sched based"
                                    : use_timer ? "Timer-based"
                                                : "Signal-based");
    printf("=====================================\n");
    ...
    while (!shutdown_requested) {
        if (use_bpf) {
            if (ttsched_wait_next_period() != 0)
                continue; /* SHM not ready or timpani-n stopped — skip workload */
        } else {
        ...
        deadline_time.tv_nsec +=
            (task_config.deadline_ms % 1000) * NSEC_PER_MSEC;
        ...
        unsigned long runtime_us =
            diff(ts_ns(now), ts_ns(before)) / NSEC_PER_USEC;
