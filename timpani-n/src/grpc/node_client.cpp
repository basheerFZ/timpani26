...
NodeClient::~NodeClient() { disconnect(); }

void NodeClient::set_table_callback(TableCallback cb)
{
    table_callback_ = std::move(cb);
}
void NodeClient::set_update_callback(UpdateCallback cb)
{
    update_callback_ = std::move(cb);
}
void NodeClient::set_shutdown_callback(ShutdownCallback cb)
{
...
void NodeClient::connect()
{
    ...
    stream_thread_ =
        std::make_unique<std::thread>(&NodeClient::stream_thread_func, this);
}
...
void NodeClient::stream_thread_func() { reconnect_loop(); }
...
void NodeClient::reconnect_loop()
{
    ...
    while (running_) {
        ...
        std::cerr << "[NodeClient] Connection lost. Reconnecting in " << backoff
                  << "s..." << std::endl;
        sleep(backoff);
...
void NodeClient::send_table_applied(const std::string& table_id, bool success,
